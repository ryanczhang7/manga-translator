...
|---|---|---|---|
| Text-region detection | `mayocream/comic-text-detector-onnx` → `comic-text-detector.onnx`, 90 MiB | a comic/manga text detector exported to ONNX (the `comic-text-detector` lineage ships one) | Emits both a `seg` mask and YOLO boxes. **They disagree**: the box head does not fire on art-integrated SFX but the mask covers them, so MT-019 must inpaint `seg ∩ accepted boxes`, never the raw mask. **Licence — SETTLED 2026-09-13 by the user, recorded as `architecture.md` D13: ship GPL-3.0 knowingly, conveying weights and not code.** MT-030 found no permissively-licensed replacement clearing the pinned floor (best permissive peak IoU 0.45 against the incumbent's 0.71–0.75), so the choice was product, not technical. **What ships is the weights file plus first-party code** — MT-002 already forbids reusing upstream's source, and the detector stays behind the `detect` adapter boundary. **Five obligations fall to MT-024**, listed in D13: GPL-3.0 licence text, component identification, a corresponding-source offer, Manga109-s attribution, and keeping notices in step with the model manifest. Note the first three are **copyleft, not attribution** — a credits line does not discharge them. **The Manga109-s objection recorded here before MT-030 was void**: Manga109-s is the 87-volume subset published expressly for commercial use and its terms permit commercial use of models trained on it; the academic-only restriction belongs to the full Manga109. MT-002 audit E6, E7; `docs/wiki/audits/MT-030-detector-licence.md`. |
