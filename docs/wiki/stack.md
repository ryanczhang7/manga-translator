...
|---|---|---|---|
| Text-region detection | `mayocream/comic-text-detector-onnx` → `comic-text-detector.onnx`, 90 MiB | a comic/manga text detector exported to ONNX (the `comic-text-detector` lineage ships one) | Emits both a `seg` mask and YOLO boxes. **They disagree**: the box head does not fire on art-integrated SFX but the mask covers them, so MT-019 must inpaint `seg ∩ accepted boxes`, never the raw mask. **Licence risk — GPL-3.0 upstream; needs a user decision before EPIC-03 ships.** MT-002 audit E6, E7. **Corrected by MT-030:** the second half of this risk as originally written — "partly trained on Manga109-s", read as an academic-only restriction — **is void.** Manga109-**s** is the 87-volume subset published expressly for commercial use, and its terms permit commercial use of models trained on it, subject only to clear attribution (which MT-024 must carry). The academic-only restriction belongs to the full Manga109. **GPL-3.0 is the entire blocker.** MT-030 found no permissively-licensed replacement that clears the pinned floor; the choice between shipping GPL-3.0 knowingly and commissioning a detector is the user's and is open. See `docs/wiki/audits/MT-030-detector-licence.md`. |
